...
export function scanProject(root) {
  ...
  for (const file of found.yaml) {
    ...
    parsed.push({ file, docs, text });
    indexServiceAccounts(docs, serviceAccounts);
  }

  const allowances = [];
  const refusedAllowances = [];
  for (const { file, docs, text } of parsed) {
    // The raw text goes in alongside the parsed documents so a manifest can carry a rule-scoped
    // opt-out. Parsed YAML has no comments and no line numbers, so without the source there is no
    // way to say "this one, for this reason" -- and a policy with no such way gets bypassed
    // wholesale, which is the argument the exception file above is already built on.
    findings.push(
      ...scanKubernetes(docs, serviceAccounts, rel(file), text, { allowances, refused: refusedAllowances })
    );
  }
  ...
  return {
    ...
    allowances,
    // A marker naming a rule but giving no reason. Reported rather than dropped, for the same
    // reason a refused exemption is: it is the only way its author learns it had no effect.
    refusedAllowances,
